package com.Sasha.rp4kmod.model;
...
public class ModelPenguin extends ModelBase
{
    ...
    public void render(Entity entity, float time, float limbSwingDistance, float f2, float headYRot, float headXRot, float yTrans) {
    	...
    	if(this.isChild){
    		...
    		GlStateManager.translate(0.0F, 24.0F * yTrans, 0.0F); //penguin would be in the air, so drops it to floor
        	beak.render(yTrans);
        	head.render(yTrans);
        	body.render(yTrans);
    ...
    private void setRotation(ModelRenderer model, float x, float y, float z) {
  	...
  	public void setRotationAngles(float time, float limbSwingDistance, float par3, float headYRot, float headXRot, float par6, Entity par7Entity){  
  		float armRotateSpeed = 999f;
  		leftarm.rotateAngleX = MathHelper.cos(time * 0.6662F) * 1.4F * limbSwingDistance * armRotateSpeed;
        rightarm.rotateAngleX = MathHelper.cos(time * 0.6662F + 3.141593F) * 1.4F * limbSwingDistance * armRotateSpeed;
        ...
        head.rotateAngleY = MathHelper.cos(time * 0.6662F) * 1.4F * limbSwingDistance * 22;
